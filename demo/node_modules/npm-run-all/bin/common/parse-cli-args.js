...
var CONFIG_OPTION = /^--([^=]+?)(?:=(.+))$/;
var PACKAGE_CONFIG_PATTERN = /^npm_package_config_(.+)$/;
var CONCAT_OPTIONS = /^-[clnprs]+$/;
...
function createPackageConfig() {
    ...
    Object.keys(process.env).forEach(function (key) {
        var m = PACKAGE_CONFIG_PATTERN.exec(key);
        if (m != null) {
...
function parseCLIArgsCore(set, args) {
    ...
    if (!set.parallel && set.race) {
        throw new Error("Invalid Option: " + (args.indexOf("--race") !== -1 ? "race" : "r") + " (without parallel)");
    }
