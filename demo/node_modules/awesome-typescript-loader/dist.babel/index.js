...
function compiler(webpack, text) {
    ...
    var depsInjector = {
        add: function add(depFileName) {
            return webpack.addDependency(path.normalize(depFileName));
        },
    ...
    try {
        ...
        if (compiledModule) {
            ...
            var transformationFunction = function transformationFunction() {
                return transform(webpack, instance, fileName, text);
            };
...
function invokeKnownFilesOneTime(instance) {
    if (instance.loaderConfig.externals && !instance.externalsInvoked) {
        instance.loaderConfig.externals.filter(deps_1.isTypeDeclaration).forEach(function (ext) {
            return instance.tsState.fileAnalyzer.checkDependencies(ext);
        });
