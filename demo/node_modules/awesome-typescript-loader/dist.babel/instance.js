...
function ensureInstance(webpack, query, instanceName) {
    ...
    var _a = readConfigFile(process.cwd(), query, tsImpl),
        configFilePath = _a.configFilePath,
        compilerConfig = _a.compilerConfig,
    ...
    return getInstanceStore(webpack._compiler)[instanceName] = {
        ...
        checker: forkChecker ? checker_1.createChecker(compilerInfo, loaderConfig, compilerConfig.options, webpackOptions, tsState.defaultLib, plugins) : null,
    ...
    compiler.plugin('after-compile', function (compilation, callback) {
        ...
        if (forkChecker) {
            ...
            diagnostics.filter(function (err) {
                return !ignoreDiagnostics_1 || ignoreDiagnostics_1.indexOf(err.code) == -1;
            }).map(function (err) {
                return "[" + instanceName + "] " + helpers_1.formatError(err);
            }).forEach(emitError);
