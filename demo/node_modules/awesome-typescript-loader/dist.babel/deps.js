...
var FileAnalyzer = function () {
    ...
    FileAnalyzer.prototype.findImportDeclarations = function (fileName) {
        ...
        var imports = [];
        imports.push.apply(imports, info.importedFiles.map(function (file) {
        ...
        imports.push.apply(imports, info.referencedFiles.map(function (file) {
            return file.fileName;
        }).map(function (depName) {
            var relative = /^[a-z0-9].*\.d\.ts$/.test(depName) ? './' + depName : depName;
        ...
        if (info.typeReferenceDirectives) {
            imports.push.apply(imports, info.typeReferenceDirectives.map(function (file) {
                return file.fileName;
            }).map(function (depName) {
